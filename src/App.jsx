...
const BAD_HABITS = [
	{
		...
		malefícios: [
			...
			"Afeta concentração",
			"Impacta saúde mental",
			"Prejudica relacionamentos",
		],
	},
	{
		id: "ifood",
		label: "iFood/Fast Food",
		icon: "🍔",
		malefícios: [
			"Aumento de peso",
			"Problemas cardiovasculares",
			"Dependência de alta gordura",
			"Aumento de gastos",
...
function getGrade(avg) {
	if (avg >= 95)
		return { label: "S", color: "#a78bfa", glow: "rgba(167,139,250,0.4)" };
	if (avg >= 80)
		return { label: "A", color: "#22c55e", glow: "rgba(34,197,94,0.4)" };
	if (avg >= 65)
		return { label: "B", color: "#fbbf24", glow: "rgba(251,191,36,0.4)" };
	if (avg >= 45)
		return { label: "C", color: "#fb923c", glow: "rgba(251,146,60,0.4)" };
...
function StatCard({ label, value, sub, color = "#3b82f6", glow }) {
	return (
		<div
			style={{
				background: "#ffffff",
...
function CustomBarTooltip({ active, payload, label }) {
	...
	return (
		<div
			...
			<div style={{ color: "#718096", marginBottom: 6, fontWeight: 500 }}>
				{label}
			</div>
			<div style={{ color: grade.color, fontWeight: 800, fontSize: 18 }}>
				{score}%
			</div>
...
export default function App() {
	...
	useEffect(() => {
		const load = async () => {
			try {
				const r = await window.storage.get("relapses");
				if (r) setRelapses(JSON.parse(r.value));
			} catch (e) {
				console.error(e);
			}
			try {
				const g = await window.storage.get("goodProgress");
				if (g) setGoodProgress(JSON.parse(g.value));
			} catch (e) {
				console.error(e);
			}
			try {
	...
	useEffect(() => {
		...
		if (goodProgress.date && goodProgress.date !== today) {
			...
			const yHabits = {};
			GOOD_HABITS.forEach((h) => {
				yHabits[h.id] = !!(goodProgress.checks || {})[h.id];
			});
			const newHistory = {
				...history,
				[goodProgress.date]: { score: yScore, habits: yHabits },
			};
			void window.storage.set("history", JSON.stringify(newHistory));
			// Reset today
			const reset = { date: today, checks: {} };
			void window.storage.set("goodProgress", JSON.stringify(reset));
		} else if (!goodProgress.date) {
			const init = { date: today, checks: {} };
			void window.storage.set("goodProgress", JSON.stringify(init));
		}
	}, [loaded, goodProgress.date, goodProgress.checks, history]);
	...
	const save = async (updated) => {
		...
		const habits = {};
		GOOD_HABITS.forEach((h) => {
			habits[h.id] = !!(updated.checks || {})[h.id];
		});
		const today = todayKey();
	...
	const togglePomodoro = (i) => {
		...
		save({
			...
			checks: {
				...checks,
				study_sessions: updated,
				study: updated.every(Boolean),
			},
		});
	};
	const toggleMeal = (mealId) => {
		...
		save({
			...
			checks: {
				...checks,
				diet_meals: updated,
				diet: DIET_MEALS.every((m) => updated[m.id]),
			},
		});
	};
	const toggleWater = (i) => {
		...
		save({
			...
			checks: {
				...checks,
				water_glasses: updated,
				water: updated.every(Boolean),
			},
		});
	};
	...
	const analytics = useMemo(() => {
		const checks = goodProgress.checks || {};
		const today = todayKey();
		const histEntries = Object.entries(history).sort(([a], [b]) =>
			a > b ? 1 : -1,
		);
		...
		for (let i = sortedDays.length - 1; i >= 0; i--) {
			...
			if (
				sortedDays[i] === expectedDate &&
				history[sortedDays[i]]?.score >= 80
			) {
				streak++;
			} else break;
		}
		...
		const last7 = Array.from({ length: 7 }, (_, i) => {
			...
			const dayName = new Date(d + "T12:00:00").toLocaleDateString("pt-BR", {
				weekday: "short",
			});
			return {
				day: dayName.replace(".", ""),
				score: entry ? entry.score : d === today ? scoreFromChecks(checks) : 0,
				date: d,
			};
		});
		const last30avg = (() => {
		...
		const habitStats = GOOD_HABITS.map((h) => {
					...
					d === today
						? {
								habits: Object.fromEntries(
									GOOD_HABITS.map((x) => [x.id, !!checks[x.id]]),
								),
							}
						: history[d];
				if (entry) {
			...
			return {
				id: h.id,
				label: h.label.replace(" 3h", "").replace("+Ω3", ""),
				icon: h.icon,
		...
		const heatmap = Array.from({ length: 84 }, (_, i) => {
			...
			const s =
				d === today ? scoreFromChecks(checks) : (history[d]?.score ?? null);
	...
	}, [history, goodProgress]);

	const checks = goodProgress.checks || {};
	const completedCount = GOOD_HABITS.filter((h) => checks[h.id]).length;
	const pomSessions =
		checks.study_sessions || Array(POMODORO_COUNT).fill(false);
			...
			{isDesktop && (
				<div
					...
					<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
						{[
							{ key: "bad", label: "🚫 Hábitos Ruins", color: "#ef4444" },
							{ key: "good", label: "✅ Hábitos Bons", color: "#10b981" },
							{ key: "stats", label: "📊 Estatísticas", color: "#3b82f6" },
						].map((t) => (
							<button
								key={t.key}
								...
								style={{
									...
									color: tab === t.key ? t.color : "#718096",
									boxShadow: tab === t.key ? `0 0 0 3px ${t.color}15` : "none",
									textAlign: "left",
								}}>
								{t.label}
							</button>
			...
			<div
				...
				<div
					style={{
						...
						marginLeft: isDesktop ? 280 : "auto",
						marginRight: isDesktop ? 0 : "auto",
						background:
							"radial-gradient(ellipse at 20% 0%, rgba(59,130,246,0.04) 0%, transparent 60%)",
						pointerEvents: "none",
						zIndex: 0,
					}}
				/>

				{/* Header - Desktop Info Section */}
				{!isDesktop && (
					<div
						style={{ position: "relative", zIndex: 1, padding: "32px 24px 0" }}>
						<div
							style={{
								fontSize: 11,
								letterSpacing: 1.5,
						...
						<div
							...
							<div
								...
								<span style={{ color: "#10b981", fontWeight: 700 }}>
									{completedCount}/{GOOD_HABITS.length}
								</span>
								<span style={{ color: "#718096", marginLeft: 6 }}>hábitos</span>
							</div>
							<div
								style={{
									background: "#eff6ff",
								...
								<span style={{ color: "#718096", marginLeft: 6 }}>streak</span>
							</div>
							<div
								style={{
									background: "#fef3c7",
						...
						{[
							...
							<button
								...
								style={{
									...
									padding: "10px 4px",
									borderRadius: 10,
									border: tab === t.key ? "1px solid #e2e8f0" : "none",
									cursor: "pointer",
									fontSize: 13,
									fontWeight: 600,
									fontFamily: "inherit",
									...
									color: tab === t.key ? "#1a202c" : "#718096",
									boxShadow:
										tab === t.key ? "0 1px 3px rgba(0,0,0,0.05)" : "none",
								}}>
								{t.label}
							</button>
						))}
				...
				<div
					style={{
						position: "relative",
						zIndex: 1,
						padding: isDesktop ? "32px 48px 48px" : "20px 24px 120px",
						...
						<div>
							<div style={{ marginBottom: 32 }}>
								...
								<p style={{ fontSize: 14, color: "#718096", margin: 0 }}>
									Rastreie seu progresso sem recaídas
								</p>
							</div>

							{/* Introdução com motivação */}
							<div
								style={{
									background: "#fee2e2",
								...
								<div
									...
									<strong>⚠️ Lembre-se:</strong> Cada dia sem recaída é uma
									vitória. Você está construindo uma versão melhor de si mesmo.
									...
									return (
										<div
											...
											{!ts ? (
												<button
													onClick={async () => {
														const u = { ...relapses, [habit.id]: Date.now() };
														setRelapses(u);
									...
									if (habit.type === "diet") {
										return (
											<ExpandableCard
												...
												onToggleExpand={() => toggleExpand("diet")}
												subtitle={
													mealsDone === 0
														? "Toque para ver refeições"
														: done
															? "Todas refeições feitas ✓"
															: `${mealsDone}/${DIET_MEALS.length} refeições`
												}
												pips={DIET_MEALS.map((m, i) => (
													<div
														key={i}
														style={{
															width: 7,
												...
												<div
														...
														return (
															<div
																...
																style={{
																	...
																	border: `1px solid ${mDone ? "rgba(245,158,11,0.25)" : "rgba(255,255,255,0.05)"}`,
																...
																<div
																	style={{
																		...
																		border: `2px solid ${mDone ? "#f59e0b" : "rgba(255,255,255,0.15)"}`,
																		display: "flex",
																		alignItems: "center",
																		justifyContent: "center",
																		flexShrink: 0,
													...
													<MiniProgress
														value={mealsDone}
														max={DIET_MEALS.length}
														label={`${mealsDone} / ${DIET_MEALS.length} refeições`}
														color="#f59e0b"
													/>
												</div>
											</ExpandableCard>
											...
											<ExpandableCard
												...
												subtitle={
													waterDone === 0
														...
														: done
															? "4L atingidos hoje ✓"
															: `${(waterDone * 0.5).toFixed(1)}L / 4L`
												}
												pips={waterGlasses.map((v, i) => (
													<div
														key={i}
														style={{
															width: 7,
												...
												<div>
															...
															<div
																...
																style={{
																	...
																	background: v
																		...
																		: "rgba(255,255,255,0.03)",
																	border: `1px solid ${v ? "rgba(56,189,248,0.3)" : "rgba(255,255,255,0.06)"}`,
																	transition: "all 0.15s",
																}}>
													...
													<MiniProgress
														value={waterDone}
														max={WATER_COUNT}
														label={`${(waterDone * 0.5).toFixed(1)}L / 4L`}
														color="#38bdf8"
													/>
													{waterDone > 0 && !done && (
														<div
															...
															Faltam{" "}
															{((WATER_COUNT - waterDone) * 0.5).toFixed(1)}L 🎯
														</div>
													)}
													{done && (
														<div
									...
									if (habit.type === "study") {
										return (
											<ExpandableCard
												...
												onToggleExpand={() => toggleExpand("study")}
												subtitle={
													pomDone === 0
														? "Toque para expandir sessões"
														: done
															? "Concluído hoje ✓"
															: `${pomDone}/${POMODORO_COUNT} sessões — ${pomDone * 30}min`
												}
												pips={pomSessions.map((v, i) => (
													<div
														key={i}
														style={{
															width: 7,
														...
														<div
															...
															style={{
																...
																background: sDone
																	? "rgba(34,197,94,0.08)"
																	: "rgba(255,255,255,0.03)",
																border: `1px solid ${sDone ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.05)"}`,
																transition: "all 0.15s",
															}}>
															<div
																style={{
																	...
																	background: sDone
																		? "#22c55e"
																		: "rgba(255,255,255,0.05)",
																	border: `2px solid ${sDone ? "#22c55e" : "rgba(255,255,255,0.15)"}`,
																	display: "flex",
																	alignItems: "center",
																	justifyContent: "center",
																	flexShrink: 0,
										...
										<div
											...
											<div style={{ flex: 1 }}>
												...
												<div
													...
													{done ? "Concluído hoje ✓" : "Toque para marcar"}
												</div>
											</div>
											<div
												style={{
									...
									<div
										...
										<div
											style={{
												height: "100%",
												borderRadius: 99,
												width: `${(completedCount / GOOD_HABITS.length) * 100}%`,
												background:
													completedCount === GOOD_HABITS.length
														? "linear-gradient(90deg,#22c55e,#4ade80)"
					...
					{tab === "stats" && (
						<div>
							...
							<div
								...
								<div
									style={{
										...
										borderRadius: 16,
										flexShrink: 0,
										background: `${overallGrade.color}12`,
										border: `3px solid ${overallGrade.color}`,
										display: "flex",
										alignItems: "center",
										justifyContent: "center",
										fontSize: 40,
								...
								<div style={{ flex: 1 }}>
									...
									<div
										...
										<span style={{ color: overallGrade.color }}>
											{analytics.avgAll}%
										</span>
									</div>
									<div style={{ fontSize: 13, color: "#718096" }}>
										Média histórica em {Object.keys(history).length} dias
									</div>
								</div>
							</div>
											...
											<Bar dataKey="score" radius={[6, 6, 0, 0]}>
												{analytics.last7.map((entry, i) => {
													...
													return (
														<Cell
															key={i}
															fill={entry.score === 0 ? "#e2e8f0" : g.color}
															opacity={entry.date === todayKey() ? 1 : 0.7}
														/>
													);
												})}
											</Bar>
											...
											{[null, 0, 40, 65, 85].map((s, i) => (
												<div
													...
													style={{
														...
														height: 10,
														borderRadius: 2,
														background: heatColor(s === null ? null : s),
													}}
												/>
											))}
											<span
							...
							<div
								...
								{[
									{
										label: "Primeira semana perfeita",
										done: analytics.bestStreak >= 7,
										icon: "🌟",
									},
									{
										label: "2 semanas seguidas",
										done: analytics.bestStreak >= 14,
										icon: "🔥",
									},
									{
										label: "1 mês seguido",
										done: analytics.bestStreak >= 30,
										icon: "💎",
									},
									{
										label: "10 dias perfeitos",
										done: analytics.perfectDays >= 10,
										icon: "⚡",
									},
									{
										label: "30 dias registrados",
										done: Object.keys(history).length >= 30,
										icon: "📅",
									},
									{
										label: "Média histórica ≥ 80%",
										done: analytics.avgAll >= 80,
										icon: "🎯",
									},
								].map((m, i) => (
									<div
										...
										style={{
											...
											gap: 14,
											padding: "12px 0",
											borderBottom: i < 5 ? "1px solid #f1f3f5" : "none",
										}}>
										...
										<div
											style={{
												...
												padding: "4px 10px",
												borderRadius: 6,
												background: m.done ? "#f0fdf4" : "#f8f9fa",
												color: m.done ? "#10b981" : "#a0aec0",
												border: `1px solid ${m.done ? "#dcfce7" : "#e2e8f0"}`,
											}}>
											{m.done ? "✓ Feito" : "Pendente"}
										</div>
									</div>
								))}
							</div>
					...
					<div
						...
						<div
							...
							<div
								...
								Isso vai zerar o contador de{" "}
								<span style={{ color: "#dc2626", fontWeight: 600 }}>
									{BAD_HABITS.find((h) => h.id === confirmReset)?.label}
								</span>
							...
							<div style={{ display: "flex", gap: 12 }}>
								...
								<button
									onClick={async () => {
										const u = { ...relapses, [confirmReset]: Date.now() };
										setRelapses(u);
										await window.storage.set("relapses", JSON.stringify(u));
